...
let baselinePackage=clone(originalPackage);
...
function persist(){
  ...
  try{localStorage.setItem(storageKey(),JSON.stringify({package:current,baselinePackage,confirmed,savedAt:new Date().toISOString()}));$('saveStatus').textContent='已保存到当前浏览器';return true}
  catch{$('saveStatus').textContent='浏览器保存失败，请导出配置';return false}
}
function changed(){confirmed=false;$('confirm').textContent='确认当前阅读层';renderPublishState();$('saveStatus').textContent='修改待保存…';renderPreview();clearTimeout(saveTimer);saveTimer=setTimeout(persist,350)}
...
function renderPreview(){
  ...
  for(const id of ['copyBody','copyTop','deliveryCopyArticle','downloadHtml','exportButton','confirm','publishArticle'])$(id).disabled=!report.valid;
  ...
  const img=current.courseMeta.assets.podcastCover;$('notesImage').hidden=!img;if(img)$('notesImage').src=img;$('podcastImage').hidden=!img;if(img)$('podcastImage').src=img;
  ...
  renderMap();renderPublishState();
}
function renderMap(){
  $('map').replaceChildren();
  [...$('article').querySelectorAll('h3')].forEach((h,i)=>{h.id='heading-'+i;const b=document.createElement('button');b.textContent=h.textContent;b.onclick=()=>focusPreview(h);$('map').append(b)});
  for(const key of KEYS.filter(k=>k!=='quote')){const b=document.createElement('button');b.textContent=(current.components[key].enabled?'编辑 · ':'已移除 · 恢复 / 编辑 · ')+current.components[key].name;b.onclick=()=>{const d=$('component-'+key);d.open=true;d.scrollIntoView({behavior:'smooth',block:'center'});focusComponent(key)};$('map').append(b)}
}
...
function renderControls(){
  const opened=[...document.querySelectorAll('#components details[open]')].map(d=>d.id);
  ...
  for(const key of KEYS){
    ...
    d.addEventListener('toggle',()=>{if(d.restoreOpen){d.restoreOpen=false;return}if(d.open&&d.isConnected)focusComponent(key)});
    ...
    toggle.type='checkbox';toggle.checked=c.enabled;toggle.setAttribute('aria-label','启用'+c.name);toggle.onclick=e=>e.stopPropagation();toggle.onchange=()=>{c.enabled=toggle.checked;state.textContent=c.enabled?'显示中':'已移除';changed()};name.textContent=c.name;summary.append(toggle,name);d.append(summary);
    ...
    if(c.hint){const hint=document.createElement('p');hint.className='muted';hint.textContent=c.hint;d.append(hint)}
    const state=document.createElement('small');state.className='muted';state.textContent=c.enabled?'显示中':'已移除';summary.append(state);
    const reset=document.createElement('button');reset.textContent='恢复建议';reset.style.marginTop='10px';reset.onclick=()=>restoreComponent(key);d.append(reset);$('components').append(d);
    if(opened.includes(d.id)){d.restoreOpen=true;d.open=true;}
  }
  renderRules();
}
function restoreComponent(key){
  const trial=clone(current),base=baselinePackage;
  trial.components[key]=clone(base.components[key]);trial.components[key].enabled=true;
  if(ANCHORS[key])trial.courseMeta.anchors[ANCHORS[key]]=base.courseMeta.anchors[ANCHORS[key]]||'';
  if(key==='concept')trial.courseMeta.conceptItems=clone(base.courseMeta.conceptItems||[]);
  if(key==='practice')trial.courseMeta.anchors.nextLesson=base.courseMeta.anchors.nextLesson||'';
  if(key==='quote')for(const rule of trial.editorialRules)rule.format=rule.recommended;
  const report=validate(trial);if(!report.valid){toast('无法恢复：'+report.errors[0]);return}
  current=trial;renderControls();changed();$('component-'+key).open=true;focusComponent(key,false);
}
function renderPublishState(){
  $('readingStage').textContent=confirmed?'✓ 阅读层已确认 → ③ 发布检查':'① 导入原课 ✓ → ② 确认阅读层 → ③ 发布检查';
  $('publishReady').classList.toggle('hidden',!confirmed);
...
$('deliveryTranscript').onclick=()=>download('transcript-review.txt',transcriptReview(current));
...
$('resetButton').onclick=()=>{if(!confirm('恢复这次构建时的版本？当前修改请先导出配置。'))return;current=clone(originalPackage);baselinePackage=clone(originalPackage);confirmed=false;renderControls();changed();persist()};
$('importButton').onclick=()=>$('importFile').click();
$('importFile').onchange=async()=>{
  const file=$('importFile').files?.[0];if(!file)return;
  try{const incoming=JSON.parse(await file.text()),r=validate(incoming);if(!r.valid)throw Error(r.errors.join('；'));if(!persist()&&!confirm('当前内容无法保存。仍要切换课程吗？请先导出备份。'))return;current=incoming;baselinePackage=clone(incoming);current.customEditorialRules??=[];renderControls();changed();persist();toast('已导入课程。相对素材路径须位于当前发布目录；迁移请用 build 命令。')}
...
$('sourceTop').onclick=()=>document.querySelector('.panel:first-child .scroll').scrollTo({top:0,behavior:'smooth'});
$('resetEditorial').onclick=()=>restoreComponent('quote');
$('publishTitle').onclick=()=>$('copyTitle').click();
$('publishSubtitle').onclick=()=>$('copySubtitle').click();
$('publishArticle').onclick=()=>$('copyBody').click();
$('publishPodcast').onclick=()=>$('copyNotes').click();
$('article').onclick=e=>{if(e.target.closest('[data-preview-audio]'))showTab('audioPane')};
...
try{const saved=JSON.parse(localStorage.getItem(storageKey())||'null');if(saved?.package&&validate(saved.package).valid){current=saved.package;baselinePackage=saved.baselinePackage&&validate(saved.baselinePackage).valid?saved.baselinePackage:(current.courseMeta.id===originalPackage.courseMeta.id?clone(originalPackage):clone(current));confirmed=!!saved.confirmed;$('saveStatus').textContent='已恢复当前浏览器草稿'}else $('saveStatus').textContent='已载入构建版本；修改将自动保存'}catch{$('saveStatus').textContent='浏览器存储不可用，请导出配置'}
